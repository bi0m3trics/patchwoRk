# Load the terra library for spatial data analysis
library(terra)
...
# Define a function to plot a raster by its name
...
# Define a function to sum rasters by suitability level
...
# Set the path to the input raster
input_raster <- "E:/fragPatch/FtValleyClipCHM.tif"

# Define the suitability levels, gap distances, and spur distances. This can be 
# to be single integers or vectors of integers.
suitList <- c(0, 2, 50)
gapList <- seq(2, 8, by = 2)
spurList <- seq(6, 12, by = 2)

# Process the rasters based on the patchmorph parameters
rasters <- process_rasters_patchmorph(input_raster, suitList, gapList, spurList)

# Call the plot_raster_by_name function to plot the raster named "suit_2_gap_2_spur_10"
plot_raster_by_name(rasters, "suit_2_gap_2_spur_10")

# Call the sum_rasters_by_suitability function to get a list of summed rasters
