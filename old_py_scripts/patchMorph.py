...
gapLow = 2 # number in map units
gapHigh = 10 # number in map units
gapNum = 5 # How many gaps do you have between Min. and Max.?
gapBy = (gapHigh - gapLow) / gapNum

spurLow = 4 # number in map units
spurHigh = 20 # number in map units
spurNum = 5 # How many spurs do you have between Min. and Max.?
spurBy = (spurHigh - spurLow) / spurNum

outDir = "D:\\fragPatch\\patchMorph"
...
# lines to reference “Spurs”.
	for cntGap in range(1, gapNum+1):
		...
		arcpy.CreateFolder_management(outDir1, "\\gap" + str(gapDist))
		outDir2 = outDir1 + "\\gap" + str(gapDist)
...
# lines to reference “Gaps”.
		for cntSpur in range(1, spurNum+1):
			...
			arcpy.CreateFolder_management(outDir2, "\\spur" + str(spurDist))
			outDir3 = outDir2 + "\\spur" + str(spurDist)
